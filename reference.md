...
## Renderer
A renderer is responsible for rendering the layout generated by Edgar.Godot into Godot nodes, typically `TileMapLayer` nodes. It supports filtering of tiles and rooms through metadata and provides signals for post-processing steps.

For custom post-processing procedures, the renderer emits the `post_process` and `markers_post_process` signals, allowing you to hook into the rendering process and modify the output as needed, with the ability waiting for the signals. 

There are also some flexible way to use renderer:
1. Script way: extends `EdgarRenderer2D` script and override `_post_process` and `_markers_post_process` function. 
> [!NOTE]
> For simplicity, you can also extends your `TileMapLayer` and implement a `_post_process(renderer: EdgarRenderer2D, id: int)` method.  
> ```gdscript
...
> ```
2. Signal way: attach `EdgarRenderer2D` node and implement functions for the signals
